...
export default function Projects() {
  ...
  return (
    <section className="py-12 md:py-24 max-w-5xl mx-auto space-y-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {projects.map((project, index) => (
          <article
            key={index}
            className="group relative flex flex-col justify-between p-6 bg-card border border-border rounded-xl transition-all hover:shadow-lg dark:hover:border-primary/50"
          >
            <div className="space-y-4 mb-6">
              <div className="flex items-start justify-between gap-4">
                ...
                {project.badge && (
                  <span className="shrink-0 text-[10px] md:text-xs font-medium px-2 py-1 bg-secondary text-secondary-foreground rounded-full">
                    {project.badge}
                  </span>
                )}
              </div>
            ...
            <a
              ...
              className={`inline-flex items-center gap-2 text-sm font-medium w-fit px-4 py-2 rounded-md transition-colors ${
                project.isPrivate
